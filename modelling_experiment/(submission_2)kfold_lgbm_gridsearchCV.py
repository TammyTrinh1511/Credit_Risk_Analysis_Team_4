...
def fit_model_kfold_lgbm_grỉdsearchCV(): 
    ...
    with timer('Submit and save the result: '):
        ...
        if not os.path.exists("../results/"):
            os.makedirs("../results/")
        submit.to_csv('../results/kfold_lgbm_gridsearchCV.csv', index=False)
    with timer('Draw important features:'):
        feat_importances_show(feature_names=columns, model=final_model, save_path='../results/kfold_lgbm_gridsearchCV.png')

if __name__ == '__main__':
    fit_model_kfold_lgbm_grìdsearchCV()